...
const ProMainVideoPage = () => {
  ...
  useEffect(() => {
    const createAnswerAsync = async () => {
      ...
      //we have audio anv video, we can make an answer and setLocalDescription
    };
    //we only create an answer if audio and video are enabled AND haveCreatedAnswer is false
    //this may run many times, but these 3 events will only happen once time
    if (
    ...
    <div className="main-video-page">
      <div className="video-chat-wrapper">
        ...
        <video
          ...
          playsInline
        ></video>
        {callStatus.audio === "off" || callStatus.audio === "off" ? (
